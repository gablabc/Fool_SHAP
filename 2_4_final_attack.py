""" 
The final part of the attack is for the company to cherry-pick S_1' with the
...
are stored in attacks/results.csv. We also put the brute-force result as a reference
point.
"""
import argparse
import glob
...
if __name__ == "__main__":
    ...
    if ohe_encoder is not None:
        ...
        mapper = lambda x: x
    # TreeSHAP on XGB explains the Logits
    if args.explainer == "tree" and args.model == "xgb":
        black_box = lambda x:model.predict(mapper(x), output_margin=True)
        f_D_0 = black_box(D_0)
        f_D_1 = black_box(D_1)
    # Otherwise we explain the Conditional Probability
    else:
        # Preprocessing converts to np.ndarray
        black_box = lambda x: model.predict_proba(mapper(x))
    ...
    biased_shap_values = np.mean(np.mean(LSV, axis=1), axis=1)
    CI_b = confidence_interval(LSV, 0.05)
